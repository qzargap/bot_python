...
array_dela = ["Хорошо", "Как всегда", "Ещё жив", "Какие дела? Я не при делах нынче!",
              ...
              "Не знаю", "Затрудняюсь ответить", "Амбивалентно", "Лучше чем вчера, но хуже чем завтра…", "Также, как и пять минут назад…",
              "Я от природы бездельник", "Ногсшибательно", "Столько не сделано, столько не сделано! А сколько еще предстоит не сделать!",
              "Хорошо! И у тебя?", "Терпимо", "Безусловно", "Всё в шоколаде, даже клавиатура!",  "Расту, цвету, старею…Всё как обычно",
...
def come_pas(message):
    bot.send_message(message.from_user.id, f'{log}, {message.text}')
    ...
    if come_users(log, message.text) == "Неправильный логин или пароль":
        global log_user
        log_user = '/'
        # bot.send_message(message.from_user.id, f'{log_user}')
    else:
        log_user = '1'
...
@bot.message_handler(content_types=['balance'])
def balance(message):
    if message.text == '/balance':
        if log_user == '1':
            bot.send_message(message.from_user.id, f'Вашь баланс {id_user(log)}')
        else:
            bot.send_message(message.from_user.id, 'Вы не вошли в систему\n /come Войти')
    else:
...
def get_text_help(message):
    if message.text == "/help":
        ...
        btn3 = types.KeyboardButton('/anime')
        btn4 = types.KeyboardButton('/sql_hi')
        btn5 = types.KeyboardButton('/registration')
        btn6 = types.KeyboardButton('/come')
        btn7 = types.KeyboardButton('/mone')
        btn8 = types.KeyboardButton('/balance')
